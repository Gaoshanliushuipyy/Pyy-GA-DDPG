...
class DDPG(object):
    def __init__(self, n_states, n_actions, action_low_bound, action_high_bound, gamma=0.99,
                 actor_lr=0.000005, critic_lr=0.000005, tau=0.01, memory_size=10000, batch_size=32):
        # actor_lr = 0.0000002, critic_lr = 0.0000001
        ...
        # self.memory = np.zeros([self.memory_size, self.n_states * 2 + 1 + 1]) # 变
    ...
    def choose_action(self, s):
        ...
        action = np.clip(np.random.normal(action, self.action_noise), self.action_low_bound, self.action_high_bound)
