...
class GeneticAlgorithm():
    ...
    def run(self, iterations=1):
        ...
        for iteration in range(iterations):
            parent_pairs = self._select_parents(self.number_of_pairs, self._get_parent_probabilities()) # 选择
            for parent_pair in parent_pairs:
                children = self._generate_children(parent_pair) # 交叉
                mutated_children = [self._mutate(child, self.mutation_rate) for child in children] # 变异
                # Possible changes here (combine with tabu search?)
                for child in mutated_children:
                    child_fitness = self.get_fitness(child) #演化评估
                    # R_arr = np.array(child_fitness)
                    # np.savetxt("./dataone/ImproveR.txt", R_arr, fmt="%f", delimiter="  ")
